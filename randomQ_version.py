...
grid-like world. Q learning is applied to search for the optimum path. We test
impact of initialiting a Q-matrix with non-zero values.
"""
...
Q_1, cummulative_Q_1, cummulative_R_1 = trainAgent(episodes,R,Q,gamma,alpha,epsilon,initial_state,final_state)
p_1 = testAgent(R,Q_1,initial_state,final_state,limit,road,markovNodes)

# Random Q values in the valid positions
Q_rand = np.random.rand(Q.shape[0],Q.shape[1])*100
Q_rand[np.where(Q_1 == 0)] = 0

Q_2, cummulative_Q_2, cummulative_R_2 = trainAgent(episodes,R,Q_rand,gamma,alpha,epsilon,initial_state,final_state)
p_2 = testAgent(R,Q_2,initial_state,final_state,limit,road,markovNodes)

# Plot steps
plt.figure(1)
plt.plot(cummulative_Q_1,label="Zero Q")
plt.plot(cummulative_Q_2,label="Random Q")
...
# Plot steps
plt.figure(2)
plt.plot(cummulative_R_1,label="Zero Q")
plt.plot(cummulative_R_2,label="Random Q")
plt.title("Performance of the agent")
plt.xlabel("Episodes")
